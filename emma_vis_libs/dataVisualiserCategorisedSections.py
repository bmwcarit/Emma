...
class CategorisedImageConsumptionList:
    ...
    def __categoriseImage(self):
            ...
            :return: A grouped dataframe containg the grouped image and modules
            """
    ...
    def displayUsedByModulesInImage(self):
        ...
        for i in range(len(self.imageSumObj.consumptionByMemType[USED_PERCENT])):
            ...
            barGraph.annotate(s=annotationUsagePercentage + "\n" + annotationUsageAbsoluteValue,
                              xy=(i-barWidth/2, self.imageSumObj.consumptionByMemType[USED_PERCENT][i] + 0.01),  # Make annotation on the left side of each bar, xy= accepts a touple of XY location
                              color="#505359")

        # TODO : This needs to be corrected because now the legend elements have different colors from the diagram content. (AGK)
        # Configure legend
    ...
    def createCategoriesSections(self):
        ...
        categoriesCSV = pandas.read_csv(csvfilepath, index_col=6, sep=";").reset_index().drop(
            [SECTION_SIZE_BYTE, MODULE_SIZE_BYTE], 1).groupby(
            [CONFIG_ID, MEM_TYPE, SECTION_NAME, CATEGORY]).agg({MODULE_NAME: "count"})
